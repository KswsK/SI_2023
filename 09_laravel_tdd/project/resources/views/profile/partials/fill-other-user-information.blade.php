<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">
            {{ __("Complete the delivery details to shorten the process of buying an item.") }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="postcode" name="postcode" type="text" class="mt-1 block w-full" :value="old('postcode', $user->postcode)" required autocomplete="postcode" />
            <x-input-error class="mt-2" :messages="$errors->get('postcode')" />
        </div>

        <div class="flex items-center gap-4">
            <x-primary-button id="3">{{ __('Save') }}</x-primary-button>
